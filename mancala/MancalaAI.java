...
public class MancalaAI {
	...
	private static int minimax(
		...
		if (! playerCanMove(board)) {
			nextPlayer(board);
			return minimax(board, player, !findBest, false, depth, alpha, beta);
		}
		...
		for (int k = 0 ; k < 2 ; k++) {
			...
			for (int i = 0 ; i < 6 ; i++) {
				...
				if (board[index] > 0 && (
					...
					next = move(board, index);
					boolean samePlayer = (next[14] == board[14]);
					
					value = minimax(next, player, 
